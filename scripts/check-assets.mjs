...
const miss = [];
for (const L of LOCATIONS) { for (const f of [`hdri/${L.id}_2k.hdr`, `backdrop/${L.id}_8k.jpg`, `backdrop/${L.id}_4k.jpg`, `thumbs/${L.id}.jpg`]) if (!fs.existsSync(`public/assets/${f}`)) miss.push(f); if (!meta[L.id]) miss.push(`backdrops.json:${L.id}`); }
